...
normal view after saving. Read views sanitize stale auth markers in memory
without writing; only an explicit write persists repaired state. See
`config-store.ts` (`readProviderView` and the provider write paths).
...
`settings.json`'s active pointer (`config-store.ts`, `listProviders`). A configured
...
the user chooses a default (`frontend/src/pages/ModelConfigPage.tsx`).
...
2. a matching `reasoning_options` entry in the local models.dev cache (kept
   in memory with only the fields these lookups read; before the first
   successful fetch, or offline, the snapshot shipped at
   `agent-assets/model-presets/models-dev-snapshot.json.gz` answers —
   regenerated by `scripts/refresh-models-dev-snapshot.ts`);
...
`fetchProviderModelsFromDraftResult()` in `config-store.ts`. The
...
exists, it clears the default pointer (`config-store.ts`, `usableActive` and
the active-pointer write path). A default is a convenience, not the only way
to choose a model.
...
configuration unusable without deleting the provider (`config-store.ts`).
...
sending until a usable model is selected (`core/alt-theory-core.ts`,
...
Fork/related-session creation passes the parent's override, including a
hand-picked thinking level, when the child is created; the child then has its
own session header and runtime state. A `thinkingLevel` in the override means
the user chose it; its absence means the resolver's model default applies.
...
(`session-service.ts`, `openManagedRuntime`). This is current recovery behavior, not a
...
- `web-server/backend-server.integration.ts` — thinking-level
  midpoint selection and model-list metadata normalization;
- `web-server/backend-server.integration.ts` — OpenCode Go provider
  family classification and retention of endpoint-returned models;
- `web-server/backend-server.integration.ts` — built-in OAuth
  resolution, exact saved model-list preservation, and OAuth fetch behavior;
- `web-server/backend-server.integration.ts` — draft/session model
  override WebSocket state;
- `web-server/config-store.test.ts` — native config persistence and
...
- `web-server/session-service.test.ts` — persisted session model
  override behavior;
- `web-server/session-service.test.ts` — a fork preserves the parent's explicit
  thinking choice as part of the copied override;
