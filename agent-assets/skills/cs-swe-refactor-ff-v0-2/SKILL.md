...
默认**不建 `project/workstreams/swe/refactors/` 目录**——fastforward 的价值就在不留存档。

例外：用户明确"这次要留个记录" → 建 `project/workstreams/swe/refactors/{YYYY-MM-DD}-{slug}/{slug}-refactor-note.md`，内容就是上面那句汇报 + 一段"做了什么 / 为什么"。不写 design / checklist。
...
切回：触发 `cs-swe-refactor-v0-2` 从 scan 开始。已改的部分要么提交保留、要么 `git restore` 回到干净状态再扫。
...
- `cs-swe-refactor-v0-2/SKILL.md` — 完整 refactor 流程
- `agent-assets/skills/cs-swe-v0-2/references/refactor-methods.md` — 完整方法库
