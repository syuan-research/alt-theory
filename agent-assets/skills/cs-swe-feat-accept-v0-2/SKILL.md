...
> 共享路径与命名约定看 `agent-assets/skills/cs-swe-v0-2/references/shared-conventions.md` 第 0 节。
...
- `agent-assets/skills/cs-swe-v0-2/references/startup-context.md` 要不要补新规约或已知坑
...
- [ ] `requirement` 空 + 新增了用户可感能力 → 触发 `requirement observation (deferred in v0.2)` **backfill** 直接落 `status: current`
- [ ] `requirement` 指向 draft req → 触发 `requirement observation (deferred in v0.2)` **update**：`draft` → `current`，按实际实现刷新用户故事 / 边界，**保留原始愿景**（愿景不被覆盖，只在文末加变更日志记录本次改动）
- [ ] `requirement` 指向 current req 且本次改了边界 / 用户故事 / pitch → 触发 `requirement observation (deferred in v0.2)` **update** 刷新
...
- [ ] 两字段都有值：
  - 打开 `project/workstreams/swe/roadmap-deferred/{roadmap}/{roadmap}-items.yaml`
...
告诉用户："验收报告已就绪，架构文档已归并，cs-swe-feat-v0-2 工作流走完。后续 BUG 走 issue 流程。"

按 `shared-conventions.md` 第 3 节收尾推荐顺序逐项一句话提示（用户说"不用"立刻跳过）：

1. 复用价值的坑点 / 经验 → "需要沉淀 learning 吗？（`engineering learning observation (deferred in v0.2)`）"
2. 长期约束 / 技术选型 → "需要归档决定吗？（`decision observation (deferred in v0.2)`）"
   - **特检**：design 第 2.5 节是否有"建议沉淀的 convention"段。有就把那条规则原文念给用户："design 2.5 建议沉淀这条 convention：『{规则一句话}』，跑通了，要不要现在 `decision observation (deferred in v0.2)` 归档？"——这种是 design 阶段就识别出的稳定模式，比一般"问问看"更应该主动提
3. 接口变更 / 用户可见行为变更 → "需要更新指南吗？（`guide update observation (deferred in v0.2)`）"
4. 库公开接口（组件 / 函数 / 命令）变了 → "需要更新 API 参考吗？（`API/libdoc observation (deferred in v0.2)`）"
5. 第 8 节有 attention.md 候选 → 逐条问"候选 X 加到 attention.md 吗？" 用户明确同意 → 触发 `startup-context update observation` 走分节归类 / 查重 / 软上限检查（不在 accept 里手写，避免和 startup-context update observation 各搞一套口径）；**一次一条**
