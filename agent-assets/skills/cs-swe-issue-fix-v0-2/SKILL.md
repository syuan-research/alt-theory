...
> 共享路径与命名约定看 `agent-assets/skills/cs-swe-v0-2/references/shared-conventions.md` 第 0 节和 `cs-swe-issue-v0-2` 的"文件放哪儿"。
...
2. **上下文读全**：analysis 全文 + report 全文 + analysis 第 1 节定位的所有代码 + `agent-assets/skills/cs-swe-v0-2/references/startup-context.md` + 沉淀目录搜索：
   - `python agent-assets/skills/cs-swe-v0-2/tools/search-yaml.py --dir project/workstreams/swe/compound --filter doc_type=trick --filter status=active --query "{关键词}"`——确认修复方式不违背已有库用法 / 模式
...
4. 读 `agent-assets/skills/cs-swe-v0-2/references/startup-context.md`
...
- [ ] **前端改动浏览器验证**（如涉及）——按 `agent-assets/skills/cs-swe-v0-2/references/startup-context.md` 的硬要求执行，不能只 typecheck
...
验证通过后在 issue 目录建 `{slug}-fix-note.md`（位置见 `cs-swe-issue-v0-2` 的"文件放哪儿"），记录完整闭环。标准路径模板和快速通道模板都在同目录 `reference.md`。
...
1. 暴露了值得复用的坑点 → "沉淀 learning？（`engineering learning observation (deferred in v0.2)`）"
2. 沉淀出长期约束 / 规约 / 技术决定 → "归档决定？（`decision observation (deferred in v0.2)`）"
3. 这个 bug 暴露了项目通用的硬约束 / 命令陷阱 / 环境设置（一两行能讲清、CodeStable 技能每次启动都该知道）→ "记到 attention.md？（`startup-context update observation`）"
4. 最后问是否代为提交。同意时按收尾提交规则执行

建议：把 issue 目录文件和代码改动放同一次提交方便追溯；"顺手发现"另开 `cs-swe-issue-report-v0-2` 处理别塞这个 PR。

修复中发现问题实际是功能缺失（不是 bug）→ 建议另开 `cs-swe-feat-v0-2`，别在 issue 工作流里偷偷做新功能。
