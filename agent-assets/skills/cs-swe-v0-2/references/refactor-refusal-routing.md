...
**路由**：
> 这次描述里有"{触发词}"属于行为改动不在 refactor 范围。建议拆两件事：行为改动走 `cs-swe-feat-v0-2`（新能力）或 `cs-swe-issue-v0-2`（bug 修），结构改动走 refactor。拆完再回来。
...
> 1. `project-architecture-update` 更新模块边界图
> 2. `decision observation (deferred in v0.2)` 记新依赖原则
...
> 1. `decision observation (deferred in v0.2)` 拍板风格规约
...
> - 模块内部本来就该拆分 → 先走 `project-architecture-update`
