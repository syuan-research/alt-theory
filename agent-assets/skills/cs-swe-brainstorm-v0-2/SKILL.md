...
> 共享路径和命名约定看 `agent-assets/skills/cs-swe-v0-2/references/shared-conventions.md`。
...
| case | 规模 | 用户状态 | 产物 |
|---|---|---|---|
| **case 1：已经够清楚** | 不限 | 一句话能说清做什么 / 为谁 / 怎么算成功 / 不做什么 | 不落盘，直接 `cs-swe-feat-design-v0-2` |
| **case 2：小需求** | 单 feature | 知道要解决什么问题，对解法 / 边界还摇摆 | `project/workstreams/swe/features/{feature}/{slug}-brainstorm.md` → `cs-swe-feat-design-v0-2` |
| **case 3：大需求，拆解 ready** | 多 feature | 心里已有大致模块划分，想直接做拆解和接口契约 | 不落盘，移交 `roadmap/workstream observation (deferred in v0.2)` |
| **case 4：大需求，想 grill** | 多 feature | 还不想拆——想先 grill、发散、产生想法存着 | `project/workstreams/swe/brainstorms/{slug}/brainstorm.md` → 之后 `roadmap/workstream observation (deferred in v0.2)` 读到 |
...
1. **扫一眼仓库**——先读 `agent-assets/skills/cs-swe-v0-2/references/startup-context.md`；Glob `project/workstreams/swe/` 发现 architecture / features / roadmap / brainstorms / compound / requirements，读架构总入口、看已有 feature 和 roadmap 和 brainstorm、搜 compound 看有没有相关坑（`--filter doc_type=learning`）；Grep 用户描述里的关键词防术语冲突。缺 startup-context.md 视为骨架不完整，不回退读外部 AI 入口
2. **是不是接续之前的工作**：
   - `project/workstreams/swe/features/` 下有名字相近的 brainstorm？`roadmap/` 下有相近子目录？`brainstorms/` 下有相关创意记录？
   - 没有 → 当新讨论
...
3. **确认这是新功能 brainstorm**——bug 走 `cs-swe-issue-v0-2`，重构走 `cs-swe-refactor-v0-2`
...
3. **成本可控**——你判断 5-30 分钟内能搭出能跑的东西。超过这个量该走 `cs-swe-feat-ff-v0-2` 直接做或拆成正式 feature

提议格式：**"这块靠想不准，我做个最小 demo 验一下 {要验的事}，5-10 分钟，OK 吗？"** 用户秒过 / 拒绝即可。

**spike 落地约定**：
- case 2：实验代码扔 `project/workstreams/swe/features/{feature}/` 下（和 brainstorm note 同目录），文件随便起名（`spike.py` / `try-{topic}.ts`）
- case 4：spike 放 `project/workstreams/swe/brainstorms/{slug}/`，跟 brainstorm.md 挨着
...
**处理**：
1. 告诉用户"这块你已经想清楚了：{AI 一句话复述}。建议直接 `cs-swe-feat-design-v0-2`——brainstorm 对你没增量"
2. **看聊过程有没有非琐碎技术决策**——讨论了具体库选型 / Schema / 接口形态 / 跨模块约定，落一份精简 brainstorm（只填"已敲定的设计点"那节）让 design 直接读到不必重讨；纯方向确认没聊技术细节就裸退不落盘
3. 停下来等用户触发 design

**退出**："直接触发 `cs-swe-feat-design-v0-2` 从零写 design"（不落盘）；轻量落盘则"下一步 `cs-swe-feat-design-v0-2` 会读到 `{路径}` 不必重述"
...
**落盘**：收敛完成后写 `project/workstreams/swe/features/{feature}/{slug}-brainstorm.md`。
...
**退出**：主动问"这块够清楚了可以进 design 吗？"，确认后落盘。如果愿景（用户故事 / 痛点 / 边界）已经聊透了，提示用户可以先 `requirement observation (deferred in v0.2) draft` 把愿景落成 requirement，design 会读到这份 req 做对齐。告诉用户"下一步 `cs-swe-feat-design-v0-2` 会读到 `{路径}`"
...
**处理**：
1. 告诉用户"听起来是多个 feature 的集合，单 feature 装不下。`roadmap/workstream observation (deferred in v0.2)` 会做拆解和依赖梳理，我把讨论交给它"
...
**退出**："移交给 `roadmap/workstream observation (deferred in v0.2)`"（附聊到的要点汇总），不落盘
...
路径：`project/workstreams/swe/brainstorms/{slug}/`

```
project/workstreams/swe/brainstorms/{slug}/
└── brainstorm.md    创意记录
```
...
**和 roadmap 的衔接**：`roadmap/workstream observation (deferred in v0.2)` 启动时会搜 `project/workstreams/swe/brainstorms/` 看有没有相关 brainstorm。如果有，roadmap 把 brainstorm 当输入材料读，不重复分诊直接拆。

**退出**：落盘后告诉用户"想法存到 `{路径}` 了，准备好了就触发 `roadmap/workstream observation (deferred in v0.2)`，它会读到这份脑暴记录"。如果 grill 过程中愿景（用户故事 / 痛点 / 边界）已经比较清楚了，提示用户可以先 `requirement observation (deferred in v0.2) draft` 把愿景落成 requirement，后续 roadmap 拆解和 design 都有稳定对齐基准
