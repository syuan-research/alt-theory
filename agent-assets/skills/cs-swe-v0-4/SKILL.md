---
name: cs-swe-v0-4
description: Active Alt Theory v0.4 CodeStable-derived software-engineering skill bundle. Use for coding features, bugs/issues, behavior-preserving refactors, and multi-feature SWE plans when Codex needs concrete design, implementation, acceptance, issue, refactor, or swe-plan guardrails in the 2026-06-08 restructured dev tree. Use when work must choose concrete workstream roots, distinguish brainstorm from plan-record/issue/compound candidates, or respect observation boundaries. Do not use for research/eval planning, theory notes, prompt/KB authoring, agent soul/profile work, or general session continuity unless the immediate task includes code-level SWE work.
---

# CS-SWE v0.4

Current revision: v0.4 action-for-reflection. The folder name is
`cs-swe-v0-4`; internal references are relative paths inside this folder.
...
- Agent-asset archives belong in ignored `_archives/`. `project/private/` is
  optional unsorted temporary user storage, not a formal SWE artifact root.

## v0.4 Decisions

- Do not create or route to the retired generic `project/workstreams/swe/`
  container. Pick a concrete workstream first.
- Workstream continuity records use
  `project/workstreams/{workstream}/notes-and-status/`.
- SWE feature/issue/refactor artifacts use
  `project/workstreams/{workstream}/swe/` only when that subfolder exists or
  has been deliberately created for that workstream.
- Central brainstorm records are flat files:
  `project/brainstorms/YYYY-MM-DD-brainstorm-{slug}.md`.
- `observation` is a user-requested `notes-and-status/` entry type. It is not
  a SWE issue and does not automatically trigger root-cause/fix work.
- Compound output is user-controlled. Agents may identify a compound-worthy
  candidate, but do not write `project/compound/` unless the user explicitly
  asks for a decision, learning, trick, or explore artifact.
- High-fidelity uncertainty is not a user-decision prompt. When the user cannot
  decide without concrete details, examples, specs, UI, runtime behavior, or
  project conventions, use explore, probe, prototype, or action-for-reflection
  before asking for a final decision.
...
The point of this layout is version maintenance: internal links stay relative when the whole `cs-swe-v0-4/` folder is copied or revised.
